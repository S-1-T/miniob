...
void DefaultStorageStage::handle_event(StageEvent *event) {
  ...
  case SCF_INSERT: { // insert into
      ...
      rc = handler_->insert_records(current_trx, current_db, table_name, inserts.tuple_num, inserts.tuples);
  ...
  case SCF_CREATE_INDEX: {
      ...
      rc = handler_->create_index(current_trx, current_db, create_index.relation_name, 
                                  create_index.index_name, const_cast<const char**>(create_index.attribute_names), create_index.attribute_num, create_index.is_unique);
      snprintf(response, sizeof(response), "%s\n", rc == RC::SUCCESS ? "SUCCESS" : "FAILURE");
    }
    break;
