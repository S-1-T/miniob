...
RC DefaultHandler::create_index(Trx *trx, const char *dbname, const char *relation_name, const char *index_name, const char *attribute_names[], size_t attr_num, bool is_unique) {
  Table *table = find_table(dbname, relation_name);
  if (nullptr == table) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
  return table->create_index(trx, index_name, attribute_names, attr_num, is_unique);
}
...
RC DefaultHandler::insert_records(Trx * trx, const char *dbname, const char *relation_name, int tuple_num, const InsertTuple *tuples) {
  Table *table = find_table(dbname, relation_name);
  if (nullptr == table) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
  if (tuple_num == 1) {
    const InsertTuple &tuple = tuples[0];
    return table->insert_record(trx, tuple.value_num, tuple.values);
  }
